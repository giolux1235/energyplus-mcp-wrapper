...
class ThermalIDFParser:
    """Thermal IDF parser that extracts R-values and thermal properties"""
    
    def parse_idf_content(self, idf_content):
        """Parse IDF content with thermal properties extraction"""
        try:
            print(f"=== THERMAL PARSING (Length: {len(idf_content)}) ===")
            ...
            zones = self._extract_zones_thermal(idf_content)
            ...
            return {
                ...
                "parsing_details": {
                    ...
                    "material_objects": thermal_properties.get("materials_count", 0),
                    "construction_objects": thermal_properties.get("constructions_count", 0),
                    ...
                    "thermal_details": thermal_properties
                }
            }
    ...
    def _extract_zones_thermal(self, content):
        """Extract zones with area calculation"""
        zones = []
        processed_zones = set()
        ...
        for zone_name in zone_matches:
            ...
            if area_match:
                ...
                area = self._extract_zone_geometry_area_thermal(content, zone_name)
    ...
    def _extract_zone_geometry_area_thermal(self, content, zone_name):
    ...
    def _extract_thermal_properties(self, content):
        """Extract thermal properties including R-values"""
        thermal_props = {
            "materials": [],
            "constructions": [],
            "windows": [],
        ...
        for match in material_matches:
            ...
            # Calculate R-value: R = thickness / conductivity
            try:
        ...
        for match in window_material_matches:
            ...
            # Calculate U-value for windows (approximate)
            try:
                thickness_val = float(thickness)
                u_value = 1.0 / (0.025 + thickness_val / 0.04)  # Approximate U-value calculation
            except ValueError:
                u_value = 5.0  # Default U-value
    ...
    def _extract_lighting_thermal(self, content):
    ...
    def _extract_equipment_thermal(self, content):
    ...
    def _extract_occupancy_thermal(self, content):
    ...
    def _extract_refrigeration_systems_thermal(self, content):
        ...
        if not refrigeration_powers:
            ...
            if building_match:
                ...
                if any(word in building_name for word in ['retail', 'store', 'supermarket', 'grocery']):
                    ...
                    zones = self._extract_zones_thermal(content)
                    total_area = sum(zone.get('area', 0) for zone in zones)
    ...
    def _extract_hvac_thermal(self, content):
        """Extract HVAC systems"""
        hvac_systems = []
...
class ThermalEnergySimulator:
    """Thermal energy simulation with R-value based calculations"""
    
    def __init__(self):
        self.parser = ThermalIDFParser()
    
    def simulate_from_idf(self, idf_content):
        """Run simulation using IDF content with thermal calculations"""
        print("=== STARTING THERMAL SIMULATION ===")
        building_data = self.parser.parse_idf_content(idf_content)
        return self.calculate_energy_thermal(building_data)
    
    def calculate_energy_thermal(self, building_data):
        """Calculate energy consumption with thermal envelope considerations"""
        print("=== CALCULATING ENERGY THERMAL ===")
        ...
        print(f"  - Average wall R-value: {thermal_properties.get('average_wall_r_value', 0)} m²·K/W")
        print(f"  - Average window U-value: {thermal_properties.get('average_window_u_value', 0)} W/m²·K")
        print(f"  - Thermal envelope quality: {thermal_properties.get('thermal_envelope_quality', 'unknown')}")
        ...
        # THERMAL ENVELOPE BASED heating/cooling calculation
        avg_r_value = thermal_properties.get('average_wall_r_value', 0)
        avg_u_value = thermal_properties.get('average_window_u_value', 0)
        thermal_quality = thermal_properties.get('thermal_envelope_quality', 'unknown')
        
        # Adjust heating/cooling based on thermal envelope quality
        if building_type == "retail":
            # Base values for retail buildings
            base_heating = area * 5  # 5 kWh/m²/year (minimal heating in Miami)
            base_cooling = area * 120  # 120 kWh/m²/year (high cooling due to refrigeration + Miami climate)
            
            # Adjust based on thermal envelope quality
            if thermal_quality == "excellent":
                heating_energy = base_heating * 0.7  # 30% reduction
                cooling_energy = base_cooling * 0.8  # 20% reduction
            elif thermal_quality == "good":
                heating_energy = base_heating * 0.85  # 15% reduction
                cooling_energy = base_cooling * 0.9  # 10% reduction
            elif thermal_quality == "average":
                heating_energy = base_heating * 1.0  # No adjustment
                cooling_energy = base_cooling * 1.0  # No adjustment
            elif thermal_quality == "poor":
                heating_energy = base_heating * 1.2  # 20% increase
                cooling_energy = base_cooling * 1.15  # 15% increase
            else:
                heating_energy = base_heating
                cooling_energy = base_cooling
        else:
        ...
        # Performance rating with thermal considerations
        energy_intensity = total_energy / area if area > 0 else 0
        
        # Adjust performance rating based on thermal envelope quality
        if building_type == "retail":
        ...
        # Adjust performance score based on thermal envelope quality
        if thermal_quality == "excellent":
        ...
        print(f"Thermal calculated values:")
        ...
        return {
            ...
            "calculation_method": "thermal_parsing",
...
def handle_request(conn, addr):
    try:
        ...
        simulator = ThermalEnergySimulator()
        ...
        if path == '/healthz' or path == '/health':
            ...
            response_data = {
                ...
                "simulation_engine": "Thermal EnergyPlus parser with R-value extraction and thermal envelope analysis",
                "idf_parsing": True,
                "file_upload": True,
                "large_payload_support": True,
                "thermal_parsing": True,
            ...
            if method == 'GET':
                # Default simulation
                result = simulator.calculate_energy_thermal({
                    "type": "default",
                    "area": 0,
                    "lighting": 0,
                    "equipment": 0,
                    ...
                    if 'multipart/form-data' in request:
                        ...
                        try:
                            ...
                            if 'idf_content' in data:
                                ...
                                print(f"Thermal simulation result: {result['building_type']} - {result['total_energy_consumption']} kWh")
                            else:
...
def main():
    ...
    print(f"Starting THERMAL EnergyPlus MCP server on port {port}")
    ...
    print(f"THERMAL EnergyPlus MCP server listening on 0.0.0.0:{port}")
    ...
    print("THERMAL PARSING: ACTIVE")
    print("R-VALUE EXTRACTION: ACTIVE")
    print("THERMAL ENVELOPE ANALYSIS: ACTIVE")
    print("Version: 12.0.0 - THERMAL VERSION")
